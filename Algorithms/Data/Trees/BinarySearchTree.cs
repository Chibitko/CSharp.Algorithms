...
namespace Algorithms.Data.Trees
{
    ...
    public class BinarySearchTree<TKey, TValue> : BinaryTree<BinarySearchTree<TKey, TValue>, KeyValuePair<TKey, TValue>>
    {
        ...
        /// </summary>
        /// <param name="key">The root node's key</param>
        /// <param name="value">The root node's data value</param>
        /// <param name="comparer">The tree keys comparer</param>
        public BinarySearchTree(TKey key, TValue value, IComparer<TKey> comparer)
        {
        ...
        public BinarySearchTree(TKey key, TValue value)
            : this(key, value, Comparer<TKey>.Default)
        {
        }
        ...
        public BinarySearchTree<TKey, TValue> Find(TKey key)
        {
            ...
            while (current != null)
            {
                var c = m_cmp.Compare(key, current.Value.Key);
                if (c == 0)
                {
        ...
        public void BreadthFirstSearch(Func<BinarySearchTree<TKey, TValue>, bool> node)
        {
            ...
            var queue = new Queue<BinarySearchTree<TKey, TValue>>();
            queue.Enqueue(this);
            while (queue.Count > 0)
            {
        ...
        public static bool Add(ref BinarySearchTree<TKey, TValue> tree, TKey key, TValue value, IComparer<TKey> comparer = null)
        {
            if (key == null)
            {
                throw new ArgumentNullException(nameof(key));
            ...
            var current = tree;
            var parent = (BinarySearchTree<TKey, TValue>) null;
            var c = 0;
            ...
            var node = new BinarySearchTree<TKey, TValue>(key, value, cmp);
            if (parent == null)
            {
        ...
        public static bool Remove(ref BinarySearchTree<TKey, TValue> tree, TKey key, IComparer<TKey> comparer = null)
        {
            if (tree == null)
            {
